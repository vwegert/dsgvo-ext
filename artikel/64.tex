...
\begin{enumerate}
  ...
  \item In den in den Absätzen \hyperref[itm:64-1]{1} und \hyperref[itm:64-2]{2} genannten Fällen gibt der Ausschuss
   ...
   Mitglieder des \hyperref[ch:68]{Ausschusses} angenommen. Diese Frist kann unter Berücksichtigung der Komplexität der Angelegenheit um
   weitere sechs Wochen verlängert werden. Was den in \hyperref[itm:64-1]{Absatz 1} genannten Beschlussentwurf angeht,
   der gemäß \hyperref[itm:64-5]{Absatz 5} den Mitgliedern des \hyperref[ch:68]{Ausschusses} übermittelt wird, so wird angenommen, dass
  ...
  \item Der Vorsitz des \hyperref[ch:68]{Ausschusses} unterrichtet unverzüglich auf elektronischem Wege%
  \label{itm:64-5}

  \begin{enumerate}
  
    \item unter Verwendung eines standardisierten Formats die Mitglieder des \hyperref[ch:68]{Ausschusses} und die Kommission über alle
     zweckdienlichen Informationen, die ihm zugegangen sind. Soweit erforderlich stellt das Sekretariat des \hyperref[ch:68]{Ausschusses}
  ...
  \item Die in \hyperref[itm:64-1]{Absatz 1} genannte \hyperref[itm:04-21]{Aufsichtsbehörde} trägt der Stellungnahme des
   \hyperref[ch:68]{Ausschusses} weitestgehend Rechnung und teilt dessen Vorsitz binnen zwei Wochen nach Eingang der Stellungnahme auf
  ...
  \item Teilt in \hyperref[itm:64-1]{Absatz 1} genannte die zuständige Aufsichtsbehörde dem Vorsitz des \hyperref[ch:68]{Ausschusses}
   innerhalb der Frist nach \hyperref[itm:64-7]{Absatz 7} des vorliegenden Artikels unter Angabe der maßgeblichen
   Gründe mit, dass sie beabsichtigt, der Stellungnahme des \hyperref[ch:68]{Ausschusses} insgesamt oder teilweise nicht zu folgen, so
